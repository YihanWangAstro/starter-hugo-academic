---
# An instance of the Blank widget.
# Documentation: https://docs.hugoblox.com/page-builder/
widget: blank

# This file represents a page section.
headless: true

# Order that this section appears on the page.
weight: 50

title: 'Talks'
subtitle:

design:
  columns: '1'
---

<pre style="font-size:0.85em; line-height:1.8;">
talks/
├── 2026/
│   ├── Astronomy Seminar, TDLI (invited) — Apr
│   ├── 4th Seminar on Accretion-modified Stars in AGN Disks, USTC (invited) — Apr
│   └── Graduate Seminar, HKIAA, The University of Hong Kong (invited) — Mar
├── 2025/
│   └── Graduate Seminar, University of Wisconsin–Madison (invited) — Oct
├── 2024/
│   ├── Transient Phenomena and Physical Processes Around SMBHs (invited) — Oct
│   ├── Astronomy Seminar, IAS, Tsinghua University (invited) — Sep
│   ├── Astronomy Seminar, IHEP, Chinese Academy of Sciences (invited) — Sep
│   ├── Astronomy Seminar, Fudan University (invited) — Aug
│   ├── Astronomy Seminar, USTC (invited) — Aug
│   ├── Astronomy Seminar, Huazhong University of Science and Technology (invited) — Aug
│   ├── Graduate Seminar, Zhejiang University (invited) — Aug
│   ├── 50 Years of Binaries and Disks: Lubow@75 — Apr
│   └── Anticipating the Rising Tide of TDEs, KITP — Apr
├── 2023/
│   ├── Graduate Seminar, Nanjing University (invited) — Jun
│   ├── AGN Santa Fe Conference, Los Alamos National Laboratory (invited) — Mar
│   ├── Graduate Seminar, Stony Brook University (invited) — Feb
│   ├── Astro-coffee, IAS, Princeton University (invited) — Feb
│   └── Bahcall Lunch Talk, IAS, Princeton University (invited) — Feb
├── 2022/
│   ├── Graduate Seminar, Georgia Institute of Technology (invited) — Sep
│   └── 53rd Annual DDA Meeting, CCA, Flatiron Institute — Apr
├── 2021/
│   ├── Astrophysics Seminar, University of the Balearic Islands (invited) — Oct
│   ├── Planet Formation Group Meeting, CCA, Flatiron Institute — Oct
│   ├── Astronomy Seminar, Universidad de Concepción (invited) — Sep
│   ├── Astronomy Seminar, Stony Brook University — Aug
│   └── Compact Object Group Meeting, CCA, Flatiron Institute — Mar
└── 2018/
    ├── Astronomy Seminar, American Museum of Natural History — Aug
    └── Astronomy Group Meeting, Cornell University (invited) — Jun
</pre>
